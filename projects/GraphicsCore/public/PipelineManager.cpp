...
namespace Drama::Graphics::DX12
{
    ...
    {
        /*GraphicsPipelineDesc defaultPipelineSettings{};
        defaultPipelineSettings.name = "DefaultPipeline";
        create_graphics_pipeline(defaultPipelineSettings);
        m_graphicsPipelineNameToIndex[defaultPipelineSettings.name] = m_graphicsPipelines.size();
        m_graphicsPipelines.emplace_back(std::move(defaultPipelineSettings));*/
    }
    void PipelineManager::create_graphics_pipeline(GraphicsPipelineDesc& desc)
    {
        desc;
//        HRESULT hr = S_OK;
//
...
//        pipelineDesc.pRootSignature = desc.rootSignature.Get();
...
//                &pipelineDesc, IID_PPV_ARGS(&desc.pso[i]));
//            // 生成できたかチェック 失敗ならアサート
//            if (FAILED(hr))
//            {
