...
namespace Drama::Graphics
{
    namespace
    {
        constexpr uint32_t k_defaultObjectCapacity = 1024;
    }
    ObjectWorldResource::~ObjectWorldResource()
    {
        // 1) 明示破棄に委譲する
        destroy();
    }
    Core::Error::Result ObjectWorldResource::initialize(DX12::ResourceManager& resourceManager, uint32_t framesInFlight)
    {
        ...
        if (m_capacity == 0)
        {
            m_capacity = k_defaultObjectCapacity;
        }
        m_copyBytes = static_cast<uint64_t>(m_capacity) * sizeof(ObjectData);
    ...
    void ObjectWorldResource::destroy()
    {
        // 1) ディスクリプタを解放する
        // 2) バッファを破棄して参照を初期化する
    ...
    void ObjectWorldResource::add_passes(FrameGraph& frameGraph)
    {
    ...
    Core::Error::Result ObjectWorldResource::create_buffers(uint32_t framesInFlight)
    {
        // 1) 必要数を確保して初期化する
        // 2) SRV テーブルを作成する
        if (!m_resourceManager)
        {
            return Core::Error::Result::fail(
                ...
                0,
                "ObjectWorldResource has invalid dependencies.");
        }
        ...
        {
            uint32_t uploadIndex = m_resourceManager->create_upload_buffer<ObjectData>(m_capacity, L"ObjectUpload");
            DX12::DescriptorAllocator::TableID table{};
            if (m_transformBufferMode == TransformBufferMode::DefaultWithStaging)
            {
                uint32_t defaultIndex = m_resourceManager->create_structured_buffer<ObjectData>(m_capacity, L"ObjectDefault");
